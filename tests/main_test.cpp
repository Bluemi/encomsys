...
void print_player(player_relation::as_ref& player) {
	std::cout << "Player (name=" << std::get<player_name_t&>(player).name << ", x=" << std::get<position_t&>(player).x << ", y=" << std::get<position_t&>(static_cast<const std::tuple<player_name_t&, position_t&>&>(player)).y << ")" << std::endl;
}
